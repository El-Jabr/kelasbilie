...
export async function callGeminiApi(prompt: string, maxRetries = 2): Promise<string> {
  ...
  while (attempt <= maxRetries) {
    try {
      const response = await $fetch<{ candidates?: { content?: { parts?: { text?: string }[] } }[] }>(endpoint, {
        method: 'POST',
        headers: {
      ...
      const err = error as { data?: unknown, message?: string, status?: number }
      console.error(`Gemini API Error (Attempt ${attempt + 1}):`, err?.data || err?.message)
