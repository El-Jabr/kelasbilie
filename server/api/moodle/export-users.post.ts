...
export default defineEventHandler(async (event) => {
  ...
  const body = (await readBody<ExportUsersBody>(event)) || {}
  ...
  // 0. Cek Koneksi & Token Moodle di awal sebelum memproses data besar
    try {
      ...
      const errObj = err as { statusMessage?: string, message?: string }
      throw createError({
        statusCode: 400,
        statusMessage: errObj.statusMessage || errObj.message || 'Koneksi ke Moodle gagal. Periksa URL dan Token di Pengaturan Sekolah.'
      })
    }
    ...
    let teachers: TeacherWithRelations[] = []
    ...
    // Process Students
    const classroomTeachingsMap: Record<string, number[]> = {}
    ...
    if (usersToCreate.length > 0) {
      try {
        ...
        const errObj = err as { statusMessage?: string }
        console.error('Gagal membuat user di Moodle (batch), mencoba per-user:', err)
        if (errObj.statusMessage && (errObj.statusMessage.includes('[accessexception]') || errObj.statusMessage.includes('[invalidtoken]'))) {
          throw err
        }
        for (const uParam of usersToCreate) {
          try {
            ...
            const singleErrObj = singleErr as { statusMessage?: string }
            console.error(`Gagal membuat user single [${uParam.username}]:`, singleErr)
            if (singleErrObj.statusMessage && (singleErrObj.statusMessage.includes('[accessexception]') || singleErrObj.statusMessage.includes('[invalidtoken]'))) {
              throw singleErr
            }
          }
        }
          ...
          try {
            ...
            for (const en of batch) {
              try {
                ...
                const eObj = e as { statusMessage?: string }
                if (eObj.statusMessage && (eObj.statusMessage.includes('[accessexception]') || eObj.statusMessage.includes('[invalidtoken]'))) {
                  throw e
                }
    ...
    const errObj = error as { statusMessage?: string, message?: string }
    // Log error to SyncLog
    const errorMessage = errObj.statusMessage || errObj.message || 'Gagal mengekspor atau enrol user.'
    await prisma.syncLog.create({
      data: {
        resource: 'USER',
        status: 'ERROR',
