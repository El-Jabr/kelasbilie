...
const _columns = computed<any[]>(() => [
  ...
  { accessorKey: 'sas', header: sasLabel.value },
...
<template>
  <div class="space-y-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-1.5 flex-wrap">
            <span>{{ teaching?.subject?.name }} ({{ teaching?.subject?.code }}) • Kelas {{ teaching?.classroom?.name }}</span>
            <UBadge
              ...
              variant="subtle"
            >
              {{ teaching?.semester?.academicYear?.name }} - {{ teaching?.semester?.type }} {{ teaching?.semester?.isActive ? '(Aktif)' : '' }}
            </UBadge>
          </p>
        </div>
      </div>
    ...
    <!-- Loading State -->
    <div
      v-if="pendingTeaching || pendingSummary"
    ...
    >
      ...
      <p class="text-sm font-medium text-gray-600 dark:text-gray-300">
        Memuat rekapitulasi nilai siswa...
      </p>
    </div>
    ...
    <UCard v-else>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Formula: 50% Avg PH + 25% STS + 25% {{ sasLabel }}
            </p>
          </div>
      ...
      <div
        v-if="studentSummaries.length === 0"
